...
public class RedisUtil {
    ...
    public Boolean expire(String key) {
        Duration timeout = Duration.between(LocalDateTime.now(), LocalDateTime.MAX);
        return expire(key, timeout);
    }

    public Boolean expire(String key, LocalDateTime timeout) {
        ZoneId zoneId = ZoneId.systemDefault();
        ZonedDateTime zdt = timeout.atZone(zoneId);
        Date date = Date.from(zdt.toInstant());
        return expire(key, date);
    }
     ...
     * @return
     */
    public Boolean expire(String key, Date timeout) {
        return stringRedisTemplate.expireAt(key, timeout);
    }
    ...
    public String get(String key, String defaultVal) {
        return StringUtils.defaultString(get(key), defaultVal);
    }
     ...
     * @return
     */
    public BitSet getAndBit(String... key) {
        return getBit(BitOpr.AND, key);
    }

    public BitSet getBit(BitOpr opr, String... key) {
        String bit = (String) stringRedisTemplate.opsForValue().get(key[0]);
        BitSet bitSet = BitSet.valueOf(bit.getBytes());
    ...
    public void setEx(String key, String value) {
        Duration duration = Duration.between(LocalDateTime.now(), LocalDateTime.MAX);
        setEx(key, value, duration);
    }

    public void setEx(String key, String value, Duration timeout) {
        // if (timeout.toMillis() % 1000 != 0) {
        // setEx(key, value, timeout.toMillis(), TimeUnit.MILLISECONDS);
        // } else {
        // setEx(key, value, timeout.getSeconds(), TimeUnit.SECONDS);
        // }
        stringRedisTemplate.opsForValue().set(key, value, timeout);
    }

    public void setExByDays(String key, String value, long timeout) {
        setEx(key, value, timeout, TimeUnit.DAYS);
    }

    public void setExByHours(String key, String value, long timeout) {
        setEx(key, value, timeout, TimeUnit.HOURS);
    }

    public void setExByMinutes(String key, String value, long timeout) {
        setEx(key, value, timeout, TimeUnit.MINUTES);
    }

    public void setExBySeconds(String key, String value, long timeout) {
        setEx(key, value, timeout, TimeUnit.SECONDS);
    }

    public void setExByMilliseconds(String key, String value, long timeout) {
    ...
    /**
     * 自增，有效时间到0点 ,可使用 RedisAtomicLong 代替
     *
     * @param key
     * @param timeout
     * @return
     */
    public Long incr(String key, boolean timeout) {
    ...
    public Long incr(String key, LocalDateTime timeout) {
        Long incr = incr(key);
        expire(key, timeout);
        return incr;
    }

    public Long incr(String key, Date timeout) {
        Long incr = incr(key);
        expire(key, timeout);
        return incr;
    }

    public Long incr(String key, Duration timeout) {
        Long incr = incr(key);
        expire(key, timeout);
        return incr;
    }
    ...
    /**
     * 增加(自增长), 负数则为自减 ,可使用 RedisAtomicLong 代替
     *
     * @param key   自增键
     * @param delta 步长
     * @return
